...
namespace Hola\SDK\Authenticator;

use Hola\SDK\Authenticator;
use Hola\SDK\Exceptions\Authentication;
use Hola\SDK\Exceptions\ListAccounts;
use Hola\SDK\ResponseInterface;
...
class Cloud extends Authenticator
{
    ...
    /**
     * Return Feather (Hola and up) accounts.
     *
     * @return array
     */
    ...
    public function issueToken(...$arguments)
    {
        ...
        if (empty($this->accounts[$account_id])) {
            ...
            $response = $this->getConnector()->post('https://app.fr3on.info/' . $account_id . '/api/v1/issue-token-intent', null, [
    ...
    private function loadAccountsAndUser()
    {
        if (!$this->accounts_and_user_loaded) {
            ...
            $response = $this->getConnector()->post('https://app.fr3on.info/api/v1/external/login', null, [
                ...
                if (empty($result['is_ok'])) {
                    ...
                    if (!empty($result['accounts']) && is_array($result['accounts'])) {
                        foreach ($result['accounts'] as $account) {
                            ...
                            if ($account['class'] == 'FeatherApplicationInstance' || $account['class'] == 'Hola\Shepherd\Model\Account\Hola\FeatherAccount') {
